...
export default function HomeBannerCarrousel({carrousel}) {
	...
	return (
		<section className="h-screen bg-gradient-to-r from-[#F47521] to-red-500 text-white pb-[6rem] pt-[9.8rem] px-[4rem] relative flex HomeBannerContainer">
			<div className="container flex flex-col md:flex-row justify-center items-center">
				<div className="px-[1.2rem] md:w-1/2 text-center md:text-left textHomeBannerContainer">
					<h1 className="text-6xl font-bold mb-4 titleHomeBanner">{title}</h1>
					<p className="mb-8">{text}</p>
					<div className="w-[100%] flex gap-[3rem] btnHomeBanner">
						<Button link={link} title={textbtn.toLowerCase()} text={textbtn} />
					</div>
				</div>
